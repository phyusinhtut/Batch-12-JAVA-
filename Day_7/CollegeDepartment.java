...
//        System.out.println("\nOne subject is added to " + studentName.get(3));
        System.out.print("Enter the subject you want to add to Tom : ");
        String subject = scanner.nextLine();
        tomSubjects.add(subject);
        System.out.println("One subject is added to Tom...");


        System.out.print("\nEnter the subject you want to remove from Tom : ");
        String removeSub = scanner.nextLine();
        if(tomSubjects.contains(removeSub))
        {
            tomSubjects.remove(removeSub);
            System.out.println("One subject is removed from Tom...");
        }
        else {
            System.out.println("No Subject found!");
        }

        Iterator<String> updatedNameIterator = studentSubject.keySet().iterator();
        Iterator<Set<String>> updatedSubjectIterator = studentSubject.values().iterator();
        while (updatedNameIterator.hasNext() && updatedSubjectIterator.hasNext())
        {
            String name = updatedNameIterator.next();
            Set<String> subjects = updatedSubjectIterator.next();
            System.out.println(name + " --> " + subjects);
        }
